...
class UsersMembershipController extends Controller
{
    ...
    public function SaveMemberships(Request $request)
    {
        ...
        if($inputs['product_period_type'] =='recurring'):
        ...
        elseif($inputs['product_period_type'] =='single'):
            $package->mepr_product_limit_cycles_action = 'expire';
            //$package-> = $inputs['_mepr_expire_after'];
    ...
    public function ImportData()
    {
        ...
        //     //echo $ueserData['data']['ID'].':'.$ueserData['data']['bbb'].'<br/.';
        //     $image_url2 = $ueserData['data']['memberCoverImages'];
        ...
        //                 $usersBiosSave->minRange = $providerRate['from'];
        //             endif;
        //             if(isset($providerRate['to'])):
        //                 $usersBiosSave->maxRange = $providerRate['to'];
        //             endif;
        //         endif;
    ...
    public function GetWpMemberProfileImages()
    {
        $wpAllUsers = json_decode(file_get_contents('http://nannyparentconnection.com/wp-json/wp/v2/getusers'), true);
        foreach($wpAllUsers as $key =>$ueserData):
            //if($ueserData['data']['ID'] =='1' && $ueserData['data']['ID'] =='50') :
                $file = 'https://nannyparentconnection.com/wp-content/uploads/buddypress/members/'.$ueserData['data']['ID'];
            if(Http::get($file)) :
                try {
                    $response = Http::get($file);
                    if($response->successful()) {
                        $cover_img = 'https://nannyparentconnection.com/wp-content/uploads/buddypress/members/'.$ueserData['data']['ID'].'/cover-image'.'/';
                        $video_file_data = file_get_contents($cover_img);
                        echo $video_file_data.'<br/>';
                    }
                } catch (\Exception $ex) {
                    echo 'not found';
                }
        ...
        endforeach;
        dd();
        $membersCoverPics = array();
        foreach($wpAllUsers as $key =>$ueserData):
            //$MemberFolderUrl = 'https://nannyparentconnection.com/wp-content/uploads/buddypress/members/'.$ueserData['data']['ID'];
            $membersCoverPics[] = 'https://nannyparentconnection.com/wp-json/buddypress/v1/members/'.$ueserData['data']['ID'].'/cover';
        endforeach;

        foreach($membersCoverPics as $coverImg):
            //echo $coverImg.'<br/>';
            $curl = curl_init();
            curl_setopt_array($curl, array(
              CURLOPT_URL => $coverImg,
    ...
    public function membershipsReports(Request $request)
    {
        ...
        if(empty($input)){
            ...
            if($input['id']== 'month-tab'){
                ...
                return response()->json([
                    'data' => array('userMembershipGraph' => $userMembershipGraph, 'transactionStatus'=>$transactionStatus, 'currentRecord' =>$currentRecord,'byResponse'=>'byMonth')
                ]);
                ...
                return response()->json([
                    'data' => array('userMembershipGraph' => $userMembershipGraph, 'transactionStatus'=>$transactionStatus, 'currentRecord' =>$currentRecord,'byResponse'=>'byYear')
                ]);
                ...
                return response()->json([
                    'data' => array('userMembershipGraph' => $userMembershipGraph, 'transactionStatus'=>$transactionStatus,'byResponse'=>'byAll')
                ]);
            }
        }
    }
    ...
    public function membershipsReportFilter(Request $request)
    {
        ...
        if($input['id'] == 'month-tab'){
            ...
            return response()->json([
                'data' => array('userMembershipGraph' => $userMembershipGraph, 'transactionStatus'=>$transactionStatus, 'currentRecord' =>$currentRecord,'testTab'=>'monthTab')
            ]);
        }else if($input['id']== 'year-tab'){
            $date = $input['year'];
            if($input['membershipId'] == ''){
            ...
            return response()->json([
                'data' => array('userMembershipGraph' => $userMembershipGraph, 'transactionStatus'=>$transactionStatus, 'currentRecord' =>$currentRecord,'testTab'=>'yearTab')
            ]);
        }else if($input['id']== 'all-tab'){
            ...
            return response()->json([
                'data' => array('userMembershipGraph' => $userMembershipGraph, 'transactionStatus'=>$transactionStatus,'testTab'=>'allTab')
            ]);
        }
